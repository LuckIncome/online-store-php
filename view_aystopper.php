    ...
    <div id="block-body">       
        ...
        <div id="block-content">
                <?php 
                    ...
                    $page = (int)$_GET["page"]; // помещение в переменную $page значение страницы index.php?page=5

                    $count = mysql_query("SELECT COUNT(*) FROM table_products $query_aystopper", $link);
                    $temp = mysql_fetch_array($count); //Запрос в базу данных определяем общее количество товаров COUNT(*)

                    if ($temp[0] > 0) 
                    {
                    // $temp[0] обозначает что в этом значении у нас цифры
                    // помещаем в $tempcount общее количество товаров
                    $tempcount = $temp[0]; 

                    // Находим общее число страниц
                    $total = (($tempcount - 1) / $num) + 1; //(17 товаров - 1 / 10) + 1 = 2.6
                    $total = intval($total); // округление 2.6 = 2 будет

                    $page = intval($page); // округление index.php?page=5

                    if (empty($page) or $page < 0) $page = 1;
                    //Если index.php?page= будет пустой или index.php?page= -1, то будет index.php?page=1
                    if ($page > $total) $page = $total;
                    //Если index.php?page=3 будет больше 2.6, то будет index.php?page=2
                    ...
                    if ($temp[0] > 0) 
                    {
                        echo '
                            <div id="block-sorting">               
                                <p id="nav-breadcrumbs"><a href="index.php">Главная страница</a> \ <span>'.$name_aystopper.'</span></p>
                                <ul id="options-list">
                                    <li>Вид: </li>
                                    ...
                                    <li><a id="select-sort">'.$sort_name.'</a>
                                        <ul id="sorting-list">
                                            <li><a href="view_aystopper.php?go='.$go.'&sort=price-asc">От дешевых к дорогим</a></li>
                                            <li><a href="view_aystopper.php?go=<'.$go.'&sort=price-desc">От дорогих к дешевым</a></li>
                                            <li><a href="view_aystopper.php?go='.$go.'&sort=popular">Популярное</a></li>
                                            <li><a href="view_aystopper.php?go='.$go.'&sort=news">Новинки</a></li>
                                            <li><a href="view_aystopper.php?go='.$go.'&sort=brand">От А до Я</a></li>
                                        </ul>
                                    </li>
                                </ul>
                            </div>
